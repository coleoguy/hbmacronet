getMS <- function(tree, samps, report, n.site=NULL,new.names=F){
  ...
  while(check == T){
    ...
    original<-c(original,tree$edge[shortb,2])
    new<-c(new,demes1,demes2)
  ...
  if(new.names){
    names<-data.frame(original,new)
    names<-(names[names$original %in% 1:species,])
    names<-names[order(names$original),]
    
    output<-list(output,names)
  }
